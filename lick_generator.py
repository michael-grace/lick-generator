"""Lick Generator 
    ...
    Michael Grace (github.com/michael-grace)
    February 2021
"""
...
def lick_maths(start_freq, tempo, ornament, swing):
    ...
    notes = [
        ...
        (start_freq*1.125, 2.0/calc_tempo -
         (0.5/calc_tempo if ornament == "ACC" else 0)),
...
def index_handler():
    ...
    data = {
        "notes": [
            (f"{letter[0]}{number}",
             440 * (2 ** ((number-5) if letter[1] >= 3 else (number-4))) * (
                 (2 ** (letter[1]/12)) if letter[1] != 0 else 1)
             )
...
def generate_handler():
    ...
    print(f"Generating Lick {lick_no}")
